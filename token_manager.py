...
class KISTokenManager:
    ...
    def __init__(self, account_type: str, api_config: Dict[str, str], base_url: str):
        ...
        try:
            ...
            # 토큰 자동 갱신 시스템 연동 (로그 제거 - 과도한 디버그 메시지)
        except Exception as e:
            clean_log(f"토큰 자동 갱신 시스템 연동 실패: {e}", "WARNING")
        
        # 토큰 관리자 초기화 (로그 제거 - 과도한 정보 메시지)
    ...
    def _cleanup_old_tokens(self):
        ...
        try:
            ...
            clean_log(f"이전 토큰 파일 정리 중 오류: {e}", "WARNING")
    ...
    def _check_connection_info_changed(self) -> bool:
        ...
        try:
            ...
            if (current_config["app_key"] != cached_config.get("app_key", "") or
                ...
                clean_log(f"{self.account_type} 연결정보 변경 감지", "INFO")
                self._save_connection_info(current_config)
                return True
            ...
            clean_log(f"연결정보 변경 감지 실패: {e}", "WARNING")
            return False
    ...
    def _save_connection_info(self, config: Dict[str, str]):
        ...
        try:
            ...
            clean_log(f"연결정보 캐시 저장 실패: {e}", "WARNING")
    
    def _invalidate_token_on_config_change(self):
        """연결정보 변경 시 토큰 무효화"""
        if self._check_connection_info_changed():
            clean_log(f"{self.account_type} 연결정보 변경으로 토큰 무효화", "INFO")
            ...
            if self.auto_refresher:
                try:
                    ...
                    # 토큰 자동 갱신 시스템에 강제 갱신 요청 (로그 제거)
                except Exception as e:
                    clean_log(f"강제 토큰 갱신 요청 실패: {e}", "WARNING")
    ...
    def _load_token(self) -> Optional[TokenInfo]:
        ...
        try:
            ...
            clean_log(f"토큰 로드 실패: {e}", "WARNING")
            return None
    ...
    def _request_new_token(self) -> Optional[TokenInfo]:
        ...
        try:
            ...
            if access_token:
                ...
                clean_log(f"Daily Token Manager에서 토큰 획득 성공: {self.account_type}", "INFO")
                return token_info
            else:
                clean_log(f"Daily Token Manager에서 토큰 획득 실패: {self.account_type}", "WARNING")
                return None
            
        except Exception as e:
            clean_log(f"Daily Token Manager 연동 실패: {e}", "ERROR")
            return None
        ...
        with self._token_lock:
            ...
            if self._current_token is not None:
                ...
                if self._current_token.is_midnight_passed():
                    clean_log(f"자정 경과로 토큰 파기: {self.account_type}", "INFO")
                    self._current_token = None
                    if self.token_file.exists():
                        self.token_file.unlink()
                ...
                # 토큰 만료 확인
                    ...
                    clean_log(f"토큰 만료로 갱신 필요: {self.account_type}", "INFO")
                    self._current_token = None
                
                # 토큰 만료 임박 확인 (30분 전)
                elif self._current_token.is_near_expiry():
                    clean_log(f"토큰 만료 임박으로 갱신: {self.account_type}", "INFO")
                    self._current_token = None
            ...
            if self._current_token is None:
                ...
                if self._current_token is not None:
                    ...
                    clean_log(f"토큰 발급 실패: {self.account_type}", "ERROR")
                    return None
            
            # 5. 유효한 토큰 반환
            if self._current_token is not None:
                # 유효한 토큰 반환 (로그 제거 - 과도한 디버그 메시지)
    ...
    def force_refresh_token(self) -> Optional[str]:
        """강제 토큰 갱신"""
        with self._token_lock:
            clean_log(f"강제 토큰 갱신: {self.account_type}", "INFO")
