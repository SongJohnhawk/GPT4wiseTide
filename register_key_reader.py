#!/usr/bin/env python3
"""
Register_Key.md 파일을 읽어서 연동 정보를 파싱하는 유틸리티 (보안 강화 버전)
암호화된 Register_Key.md 파일을 SecureKeyHandler를 통해 안전하게 처리
"""
...
class RegisterKeyReader:
    ...
    def load_register_keys(self) -> Dict[str, Any]:
        """
        Register_Key.md 파일에서 모든 연동 정보 로드 (보안 강화 버전)
        ...
        try:
            ...
            if not register_key_path.exists():
                logger.warning(f"Register_Key.md 파일이 존재하지 않습니다: {register_key_path}")
                return {}
            ...
            if not content:
                logger.warning("Register_Key.md 파일이 비어있습니다")
                return {}
            ...
            if parsed_data:
                ...
                logger.info("Register_Key.md 파일 로드 성공 (일반 텍스트)")
                return parsed_data
            else:
                logger.warning("Register_Key.md 파일 파싱 실패")
                return {}
