...
class APIKeyManager:
    ...
    def __init__(self, project_root: Path = None):
        ...
        logger.info("APIKeyManager 초기화 완료")
    ...
    def load_api_keys_from_policy(self) -> bool:
        ...
        try:
            if not self.api_key_file.exists():
                logger.warning(f"API 키 파일이 존재하지 않음: {self.api_key_file}")
                return False
            ...
            if parsed_keys:
                ...
                logger.info("Policy 파일에서 API 키 정보 로드 성공")
                return True
            else:
                logger.error("API 키 파싱 실패")
                return False
