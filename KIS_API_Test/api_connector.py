    ...
    def record_call(self, status_code: int):
        ...
        with self.lock:
            ...
            if status_code == 429:
                ...
                logger.warning(f"Rate limit 발생: {self.rate_limit_count}회 (총 호출: {self.call_count}회)")
            elif status_code >= 400:
                self.error_count += 1
                logger.debug(f"API 오류 발생: HTTP {status_code} (총 오류: {self.error_count}회)")
            elif status_code == 200:
                logger.debug(f"API 성공 호출 (총 호출: {self.call_count}회)")
...
class KISAPIConnector:
    ...
    def __init__(self, config_path: str = "", is_mock: bool = True):
        ...
        self._balance_cache_duration = timedelta(minutes=10)  # 10분 캐시
    ...
    async def get_account_balance(self, force_refresh: bool = False) -> Dict[str, Any]:
        """계좌 잔고 조회 (10분 캐시 적용, output2에서 잔고 정보 추출)"""
        # 강제 갱신시 캐시 무효화
        if force_refresh:
            self.invalidate_balance_cache()
            logger.debug(f"강제 갱신 요청 - 캐시 무효화 완료 ({'모의투자' if self.is_mock else '실전투자'})")
            
        # 캐시 확인 (강제 갱신이 아닌 경우만)
        if not force_refresh and self._is_balance_cache_valid():
            logger.debug("캐시된 계좌 잔고 반환 (10분 이내)")
            return self._cached_balance
        
        logger.debug(f"계좌 잔고 새로 조회 ({'모의투자' if self.is_mock else '실전투자'}) - 캐시 만료 또는 강제 갱신")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if result:
                    ...
                    if result.get('output2'):
                        ...
                        if 'ord_psbl_cash' not in balance_data:
                            ...
                            balance_data['ord_psbl_cash'] = balance_data.get('nxdy_excc_amt', '0')
                        logger.debug(f"잔고 정보: 예수금={balance_data.get('dnca_tot_amt', 0)}원, 주문가능금액={balance_data.get('ord_psbl_cash', 0)}원")
                        
                        # 텔레그램 알림 제거 (API 블로킹 방지)
                        # 계좌 조회 완료 로그만 유지
                        cash = int(balance_data.get('dnca_tot_amt', '0'))
                        logger.debug(f"계좌 조회 완료 - 예수금: {cash:,}원, 보유종목: {len(result.get('output1', []))}개")
                        ...
                        self._cached_balance = full_result
                        self._balance_cache_time = datetime.now()
                        logger.debug("계좌 잔고 및 보유종목 캐시 업데이트 완룼")
                ...
                if "500" in str(e) and attempt < max_retries - 1:
                    ...
                    if attempt == max_retries - 1:
                        # 최종 실패시 기본값 반환 (시스템 중단 방지)
                        logger.warning("계좌 조회 최종 실패, 기본값 반환하여 시스템 계속 실행")
                        return {"ord_psbl_cash": "0", "dnca_tot_amt": "0", "tot_evlu_amt": "0"}
                    continue
                ...
                if attempt < max_retries - 1:
                    ...
                    # 시스템 중단 방지를 위해 기본값 반환
                    return {"ord_psbl_cash": "0", "dnca_tot_amt": "0", "tot_evlu_amt": "0"}
        
        # 모든 재시도 실패시 기본값 반환
        logger.warning("계좌 조회 모든 재시도 실패, 기본값 반환")
        return {"ord_psbl_cash": "0", "dnca_tot_amt": "0", "tot_evlu_amt": "0"}
    ...
    def _is_balance_cache_valid(self) -> bool:
        ...
        # 10분 경과 여부 확인
        elapsed_time = datetime.now() - self._balance_cache_time
        is_valid = elapsed_time < self._balance_cache_duration
        
        if is_valid:
            remaining_time = self._balance_cache_duration - elapsed_time
            logger.debug(f"잔고 캐시 유효: 남은 시간 {remaining_time}")
        else:
            logger.debug(f"잔고 캐시 만료: 경과 시간 {elapsed_time}")
        
        return is_valid
    
    def invalidate_balance_cache(self):
        """계좌 잔고 캐시 무효화 (매수/매도 후 호출)"""
        self._cached_balance = None
        self._balance_cache_time = None
        logger.debug("계좌 잔고 캐시 무효화")
